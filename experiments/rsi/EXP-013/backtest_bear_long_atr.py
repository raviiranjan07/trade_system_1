"""EXP-013b: Bear Market Oversold LONG — ATR Grid Search

Best from EXP-013: RSI<10 + bear market + filters (PF 2.32 OOS)
Now test ATR percentile variations to find optimal filter.

Grid:
  RSI thresholds: [8, 10, 12]
  ATR percentile min: [0, 10, 25, 40, 50, 60, 75]
  EMA separation min: [0, 0.25, 0.5, 1.0]

Run: python experiments/rsi/EXP-013/backtest_bear_long_atr.py
"""
...
# Grid
RSI_THRESHOLDS = [8, 10, 12]
ATR_MINS = [0, 10, 25, 40, 50, 60, 75]
EMA_MINS = [0, 0.25, 0.5, 1.0]
...
def main():
    print("=" * 70)
    print("EXP-013b: Bear Market Oversold LONG — ATR + EMA Grid")
    print("=" * 70)
    ...
    for period_name, start, end in [
        ...
        rdf = rdf[rdf["trades"] >= 10]  # min 10 trades
        rdf = rdf.sort_values("pf", ascending=False)

        print(f"\n  {'Config':<30} {'Tr':>4} {'Win%':>6} {'Net':>8} {'PF':>6} {'Avg':>7} {'DD':>7} {'2024':>7} {'2025':>7}")
        print(f"  {'-'*85}")
        ...
        # Also print: best by PF with min 20 trades
