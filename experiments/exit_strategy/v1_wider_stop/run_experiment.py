"""V1 + PROFIT_TARGET + LOCKED_PROFIT + wider stop (-50 tick or bar 10).

Rules:
  1. PROFIT_TARGET: tick_pnl >= 60 AND bars_held <= 5 -> exit
  2. LOCKED_PROFIT: once peak_mfe >= 20, exit when tick_pnl <= 15 -> exit at 15
  3. TIGHT_TS: bars 6+, peak-activated, 8 bps width
  4. STOP_LOSS: tick_pnl <= -50 -> exit (wider stop)
  5. TIME_EXIT: bar 10

Run: PYTHONPATH=src python experiments/exit_strategy/v1_wider_stop/run_experiment.py
"""
    ...
    while i < n_15m:
        if pos is not None:
            ...
            for t in range(s_idx, e_idx):
                ...
                # 4. TIGHT_TS (after bar 5, peak-activated)
...
def main():
    ...
    s = summarize(trades, "v15_LockedProfit_SL50_bar10")
    print_summary(s)
