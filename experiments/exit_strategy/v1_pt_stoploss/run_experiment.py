"""V1 exits + PROFIT_TARGET + STOP_LOSS. Test on ML V1.5.

Exit rules (in priority order per tick):
  1. PROFIT_TARGET: tick_pnl >= 60 AND bars_held <= 5 -> exit
  2. STOP_LOSS: tick_pnl <= -30 -> exit
  3. TRAILING_STOP / TIGHT_TS: drawdown check
  4. TIME_EXIT: bars_held >= 10

Run: PYTHONPATH=src python experiments/exit_strategy/v1_pt_stoploss/run_experiment.py
"""
    ...
    while i < n_15m:
        if pos is not None:
            ...
            for t in range(s_idx, e_idx):
                ...
                # 2. STOP_LOSS removed — now checked at bar 5 close only (see below)

                # 3. TRAILING_STOP / TIGHT_TS
            ...
            if exit_reason is None:
                ...
                # Bar 5 stop loss check (time + magnitude)
                if pos["bars_held"] == 5:
                    ...
                    if bar_close_pnl <= STOP_LOSS_BPS:
...
def main():
    ...
    logger.info("15m: %d | 1m: %d", len(test_15m), len(test_1m))
    ...
    s = summarize(trades, "v15_V1_PT60_bar5stop20")
    print_summary(s)
