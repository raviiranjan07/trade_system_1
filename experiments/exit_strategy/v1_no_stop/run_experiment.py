"""V1 + PROFIT_TARGET + LOCKED_PROFIT, NO stop loss, just bar 10 cap.

Rules:
  1. PROFIT_TARGET: tick_pnl >= 60 AND bars_held <= 5 -> exit
  2. LOCKED_PROFIT: once peak_mfe >= 20, exit when tick_pnl <= 15 -> exit
  3. TIGHT_TS: bars 6+, peak-activated, 8 bps width
  4. TIME_EXIT: bar 10

Run: PYTHONPATH=src python experiments/exit_strategy/v1_no_stop/run_experiment.py
"""
...
def main():
    ...
    s = summarize(trades, "v15_LockedProfit_NoStop_bar7")
    print_summary(s)
