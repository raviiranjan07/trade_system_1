"""V1 + PROFIT_TARGET + bar5 stop, with PEAK-ACTIVATED trailing stop.

Key change: trailing stop only activates once peak_mfe >= trailing_stop threshold.
Prevents noise-triggered exits on barely-profitable trades.

Rules:
  1. PROFIT_TARGET: tick_pnl >= 60 AND bars_held <= 5 -> exit
  2. TRAILING_STOP (activated): drawdown >= 20/30 AND peak >= 20/30 -> exit
  3. TIGHT_TS: bars 6+, drawdown >= 8 AND peak >= 8 -> exit
  4. EARLY_STOP (bar 5 close): bar_close_pnl <= -20 -> exit
  5. TIME_EXIT: bars >= 10

Run: PYTHONPATH=src python experiments/exit_strategy/v1_peak_activated/run_experiment.py
"""
    ...
    while i < n_15m:
        if pos is not None:
            ...
            for t in range(s_idx, e_idx):
                ...
                # 2. TRAILING_STOP / TIGHT_TS — PEAK ACTIVATED
                active_trail = TIGHT_BPS if pos["bars_held"] > TIGHTEN_AFTER_BAR else pos["trailing"]
                # Only activate trailing stop once peak has exceeded trailing threshold
                if pos["peak"] >= active_trail:
                    drawdown = pos["peak"] - tick_pnl
                    if drawdown >= active_trail:
                        exit_reason = "TIGHT_TS" if pos["bars_held"] > TIGHTEN_AFTER_BAR else "TRAILING_STOP"
                        exit_price = tick_price
                        exit_time = idx_1m[t]
                        break
            ...
            if exit_reason is None:
                ...
                # 4. EARLY_STOP at bar 5
                if pos["bars_held"] == EARLY_STOP_BAR:
        ...
        if i in signal_map and i + 1 < n_15m:
            ...
            pos = {
                ...
                "trailing": TRAIL_LONG if sig.direction == Direction.LONG else TRAIL_SHORT,
                "bars_held": 0,
                "peak": 0.0,
                "mfe_bps": 0.0,
                "mae_bps": 0.0,
...
def main():
    ...
    s = summarize(trades, "v15_V1_PT60_bar5stop20_peakactivated")
    print_summary(s)
