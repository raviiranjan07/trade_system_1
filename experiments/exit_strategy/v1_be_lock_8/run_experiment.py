        ...
        if pos is not None:
            ...
            for t in range(s_idx, e_idx):
                ...
                if pos["peak"] >= LOCK_ARM_BPS:
                    pos["armed"] = True
                # BE_LOCK: only arm if trade went NEGATIVE first, then recovered to >= 8
                if pos["mae_bps"] < 0 and pos["peak"] >= BE_ARM_BPS:
                    pos["be_armed"] = True
                ...
                # 2b. BE_LOCK: if peak went >= 8, exit when pnl returns to 0
                if pos["be_armed"] and not pos["armed"] and tick_pnl <= 0:
                    exit_reason = "BE_LOCK"
                    exit_price = tick_price
                    exit_time = idx_1m[t]
                    break
...
def main():
    ...
    s = summarize(trades, "v15_be_lock8")
    print_summary(s)
