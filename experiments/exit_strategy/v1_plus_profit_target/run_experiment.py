"""Test V1 exits + PROFIT_TARGET rule on ML V1.5.

Exit rules:
  1. PROFIT_TARGET: if tick_pnl >= 60 bps AND bars_held <= 5 -> exit at current price
  2. TRAILING_STOP: bars 1-5, 20/30 bps
  3. TIGHT_TS: bars 6+, 8 bps
  4. TIME_EXIT: bars >= 10

Uses 1-min ticks for bps, 15-min bars for counting.

Run: PYTHONPATH=src python experiments/exit_strategy/v1_plus_profit_target/run_experiment.py
"""
...
def run_backtest(signals, df_15m, df_1m):
    ...
    while i < n_15m:
        if pos is not None:
            ...
            # Process 1-min ticks (real-time BPS)
            for t in range(s_idx, e_idx):
                ...
                if tick_pnl > pos["peak"]:
                    pos["peak"] = tick_pnl
                if tick_pnl < pos["mae_bps"]:
                    pos["mae_bps"] = tick_pnl

                # 1. PROFIT_TARGET: peak >= 60 within 5 bars
...
def main():
    ...
    logger.info("15m: %d bars | 1m: %d bars", len(test_15m), len(test_1m))

    # ML V1.5 only
    logger.info("=== ML V1.5 + V1+PROFIT_TARGET exits ===")
    ...
    s = summarize(trades, "v15_V1_plus_PT60")
    print_summary(s)
