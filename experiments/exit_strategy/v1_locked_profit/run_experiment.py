"""V1 + PROFIT_TARGET + LOCKED_PROFIT (20 arms, 15 triggers).

Rules:
  1. PROFIT_TARGET: tick_pnl >= 60 AND bars_held <= 5 -> exit
  2. LOCKED_PROFIT: once peak_mfe >= 20, exit when tick_pnl <= 15 -> exit at 15 bps
  3. TIGHT_TS: bars 6+, peak-activated, 8 bps width
  4. EARLY_STOP (bar 5 close): pnl <= -20 -> exit
  5. TIME_EXIT: bar 10 -> exit

Run: PYTHONPATH=src python experiments/exit_strategy/v1_locked_profit/run_experiment.py
"""
...
# Rules
LOCK_ARM_BPS = 20           # peak must cross this to arm
LOCK_TRIGGER_BPS = 15       # exit when price returns to this
TIGHT_BPS = 8               # trailing width after bar 5
TIGHTEN_AFTER_BAR = 5
MAX_BARS = 10
    ...
    while i < n_15m:
        if pos is not None:
            ...
            for t in range(s_idx, e_idx):
                ...
                # Arm the locked-profit once peak crosses 20
                ...
                # 2. LOCKED_PROFIT: once armed, exit when price returns to 15 bps
                ...
                # 3. TIGHT_TS (after bar 5, peak-activated)
                if pos["bars_held"] > TIGHTEN_AFTER_BAR and pos["peak"] >= TIGHT_BPS:
...
def main():
    ...
    s = summarize(trades, "v15_LockedProfit20_15")
    print_summary(s)
