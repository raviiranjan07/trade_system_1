"""
FILTERED ENTRY + TRAILING STOP Strategy

Entry Filter:
- Only enter if cross is >= 5 bps away from EMA (avoid choppy signals)

Exit Logic:
- Trailing stop: Exit if price drops 10 bps from peak
- This lets winners run in uptrends while protecting profits

Run: python scripts/debug/filtered_trailing_backtest.py
"""
...
print("=" * 80)
print("FILTERED ENTRY + TRAILING STOP STRATEGY")
print("=" * 80)
...
TRAILING_STOP_BPS = 10  # Exit if drops 10 bps from peak
...
print(f"  Trailing stop: {TRAILING_STOP_BPS} bps from peak")
...
# =============================================================================
print("\n[3/4] Simulating trades with trailing stop...")

@njit
def simulate_trailing_stop(entry_price, direction, future_highs, future_lows,
                           trailing_bps, max_bars):
    """
    Trailing stop logic:
    - Track peak price since entry
    - Exit if drops trailing_bps from peak
    ...
    for i in range(min(len(future_highs), max_bars)):
        ...
        if direction == 1:  # LONG
            ...
            # Check trailing stop (from peak, not entry!)
...
for idx in range(len(test)):
    ...
    # Simulate trade
    exit_price, bars_held, exit_reason, peak_pnl_bps = simulate_trailing_stop(
        entry_price,
        direction,
        future_highs,
        future_lows,
...
trades_path = output_dir / "filtered_trailing_trades.csv"
...
# =============================================================================
print("\n" + "=" * 80)
print("BACKTEST RESULTS - FILTERED ENTRY + TRAILING STOP")
print("=" * 80)

print("\n--- STRATEGY PARAMETERS ---")
print(f"Entry: Cross EMA7 with >= {MIN_DISTANCE_BPS} bps distance")
print(f"Exit: Trailing stop {TRAILING_STOP_BPS} bps from peak")
print(f"Max holding: {MAX_HOLDING_BARS} bars")
...
print("✓ Entry filter reduces choppy signals")
print("✓ Trailing stop lets winners run while protecting profits")
print("=" * 80)
