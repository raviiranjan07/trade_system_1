"""Evaluate Curriculum Learning model — retrain + 26 metrics + backtest.

Reproduces the original L2-003 Stage 4A setup from code:
  - 32 features: 4 diffs (roc, rsi, rp, sma200) × 8 lookbacks
  - Label: direction_h8 (H8, binary LONG/SHORT)
  - Connected multi-task: MFE UP + MFE DOWN + Direction heads
  - Loss: MSE(mfe_up) + MSE(mfe_down) + 5.0 × BCE(direction)
  - Curriculum phasing from PLAN.md (PHASES not in saved code)

Runs both loose and strict variants.

Usage:
  PYTHONPATH=src python scripts/eval_curriculum.py
"""
...
# Curriculum phases (from PLAN.md — not in saved code)
PHASES_LOOSE = [
    ("Phase 1 (easy)", 50, 20),      # |mfe_diff| > 50 bps, 20 epochs
    ("Phase 2 (medium)", 30, 20),     # > 30 bps, 20 epochs
    ("Phase 3 (harder)", 15, 20),     # > 15 bps, 20 epochs
    ("Phase 4 (all)", 0, 40),         # all bars, 40 epochs
]

PHASES_STRICT = [
    ("Phase 1 (very easy)", 80, 30),  # |mfe_diff| > 80 bps, 30 epochs
    ("Phase 2 (easy)", 50, 25),       # > 50 bps, 25 epochs
    ("Phase 3 (medium)", 30, 25),     # > 30 bps, 25 epochs
    ("Phase 4 (all)", 0, 20),         # all bars, 20 epochs
]
...
# ============================================================
# MODEL (LSTMConnected used as flat MLP — exact from code)
# ============================================================
class LSTMConnected(nn.Module):
    """Same class as original. Used with flat 32-dim input (not sequential)."""
...
class SeqDS(Dataset):
    """Reshapes flat 32 features into (8, 4) sequence for LSTM."""
...
# ============================================================
def train_curriculum(model, phases, train_idx_all, mfe_diff, diff_aligned,
                     y_mfe_up, y_mfe_down, y_dir, val_loader):
    """Train with curriculum phasing."""
    mse_fn = nn.MSELoss()
    ...
    for phase_name, threshold, phase_epochs in phases:
        if threshold > 0:
            phase_idx = train_idx_all[mfe_diff[train_idx_all] >= threshold]
        else:
            phase_idx = train_idx_all

        print(f"  {phase_name}: {len(phase_idx)} bars, {phase_epochs} epochs")

        if len(phase_idx) < 100:
            print(f"    Skipping — too few bars")
            continue

        train_ds = SeqDS(diff_aligned, y_mfe_up, y_mfe_down, y_dir, phase_idx)
...
# ============================================================
def evaluate_variant(variant_name, phases, diff_aligned, y_mfe_up, y_mfe_down, y_dir,
                     mfe_diff, train_idx_all, val_idx, test_idx, label_dates,
                     feat_arr, common_idx, fc, lb, mfe_up_96_test, mfe_down_96_test):
    """Train + evaluate + backtest one curriculum variant."""
    ...
    # Train with curriculum
    model, best_vl = train_curriculum(
        model, phases, train_idx_all, mfe_diff,
        diff_aligned, y_mfe_up, y_mfe_down, y_dir, val_loader,
    )
    model.eval()
    print(f"  Best val_loss: {best_vl:.4f}")

    # Predict on test (direction only)
    ...
    print(f"  Test bars: {len(test_idx)}")
    ...
    # Trading backtest on ALL 2025 bars
...
def main():
    ...
    for phase_name, threshold, epochs in PHASES_LOOSE:
        if threshold > 0:
            n = (mfe_diff_h8[train_idx_all] >= threshold).sum()
        else:
            n = len(train_idx_all)
        print(f"    {phase_name} (>{threshold} bps): {n} bars ({n/len(train_idx_all)*100:.1f}%)")

    # ============================================================
    # RUN LOOSE VARIANT
    # ============================================================
    loose_metrics, loose_trades = evaluate_variant(
        "Curriculum LOOSE", PHASES_LOOSE,
        diff_aligned, y_mfe_up, y_mfe_down, y_dir,
        mfe_diff_h8, train_idx_all, val_idx, test_idx, label_dates,
        feat_arr, common_idx, fc, lb, mfe_up_96_test, mfe_down_96_test,
    )
    ...
    with run_experiment(
        ...
        model_type="LSTMConnected+Curriculum",
        dataset_version="feature_cache_cleaned_23col+labels_h8",
        primary_metric="test_confident_accuracy",
        notes="Curriculum LOOSE: phases 50/30/15/0 bps. Connected MFE heads. Honest date split.",
    ) as run:
        run.log_metrics(loose_metrics)

        print("\n=== LOOSE RESULTS ===")
        for k in sorted(loose_metrics.keys()):
            if "confusion" not in k:
                print(f"  {k}: {loose_metrics[k]}")

    # ============================================================
    # RUN STRICT VARIANT
    # ============================================================
    strict_metrics, strict_trades = evaluate_variant(
        "Curriculum STRICT", PHASES_STRICT,
        diff_aligned, y_mfe_up, y_mfe_down, y_dir,
        mfe_diff_h8, train_idx_all, val_idx, test_idx, label_dates,
        feat_arr, common_idx, fc, lb, mfe_up_96_test, mfe_down_96_test,
    )
    ...
    with run_experiment(
        ...
        params={
            ...
            "horizon": 8,
            "curriculum": "strict",
            "phases": "80/50/30/0 bps, 30/25/25/20 epochs",
            "loss": "MSE(mfe_up)+MSE(mfe_down)+5.0*BCE(dir)",
            "weight_decay": 0.0,
            "batch_size": BATCH_SIZE,
            "lr": LR,
        ...
        dataset_version="feature_cache_cleaned_23col+labels_h8",
        primary_metric="test_confident_accuracy",
        notes="Curriculum STRICT: phases 80/50/30/0 bps. Connected MFE heads. Honest date split.",
    ) as run:
        run.log_metrics(strict_metrics)

        print("\n=== STRICT RESULTS ===")
        for k in sorted(strict_metrics.keys()):
            if "confusion" not in k:
                print(f"  {k}: {strict_metrics[k]}")

    # ============================================================
    # COMPARISON
    # ============================================================
    print(f"\n{'='*70}")
    print("COMPARISON: Curriculum Loose vs Strict vs V1.5 Final")
    print(f"{'='*70}")
    print(f"  {'Variant':<25s} | {'Conf acc':>8s} {'N conf':>7s} {'Trades':>7s} {'Total bps':>10s} {'PF':>6s}")
    print(f"  {'-'*25}-+-{'-'*8}-{'-'*7}-{'-'*7}-{'-'*10}-{'-'*6}")

    la = loose_metrics.get("test_confident_accuracy", 0)
    ln = loose_metrics.get("test_n_confident", 0)
    lt = loose_metrics.get("backtest_n_trades", 0)
    lb_bps = loose_metrics.get("backtest_total_bps", 0)
    lpf = loose_metrics.get("backtest_profit_factor", 0)
    print(f"  {'Curriculum LOOSE':<25s} | {la:7.1%} {ln:7d} {lt:7d} {lb_bps:+10.0f} {lpf:6.2f}")

    sa = strict_metrics.get("test_confident_accuracy", 0)
    sn = strict_metrics.get("test_n_confident", 0)
    st = strict_metrics.get("backtest_n_trades", 0)
    sb = strict_metrics.get("backtest_total_bps", 0)
    spf = strict_metrics.get("backtest_profit_factor", 0)
    print(f"  {'Curriculum STRICT':<25s} | {sa:7.1%} {sn:7d} {st:7d} {sb:+10.0f} {spf:6.2f}")

    print(f"  {'V1.5 Final (H96 10feat)':<25s} | {'55.8%':>8s} {'735':>7s} {'785':>7s} {'+4385':>10s} {'1.52':>6s}")
    print(f"  {'Prev reported (loose)':<25s} | {'57.6%':>8s} {'—':>7s} {'—':>7s} {'—':>10s} {'—':>6s}")
    print(f"  {'Prev reported (strict)':<25s} | {'54.9%':>8s} {'—':>7s} {'—':>7s} {'—':>10s} {'—':>6s}")
