"""ML Signal Generator — MLP binary direction model (ONNX Runtime).
...
Uses ONNX Runtime for inference — no PyTorch/GPU dependency needed.
"""
...
class MLSignalGenerator:
    ...
    def __init__(self, model_path: Optional[Path] = None, scaler_path: Optional[Path] = None):
        """Load trained ONNX model and scaler parameters.

        Args:
            model_path: Path to trained .onnx model file
    ...
    def predict(self, features: np.ndarray) -> float:
        """Run model prediction via ONNX Runtime.
        ...
        logit = self.session.run(["logit"], {"features": x})[0].item()
